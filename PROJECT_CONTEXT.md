...
- **M14 (BLOCKING, do first):** the module's **e2e suite was never run and its migration was never applied** — the build environment had no Docker/Postgres. Unit tests (144 new), typecheck, lint and the Next build are all green, but `20260722160000_examination_management` is unverified against a real database and the roadmap's e2e cases (wizard end-to-end, admit card with/without photo) are outstanding.
