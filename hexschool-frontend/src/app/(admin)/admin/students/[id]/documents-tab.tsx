...
export function DocumentsTab({ studentId }: { studentId: string }) {
  ...
  const docs = useQuery({
    ...
    queryFn: () => studentsApi.listDocuments(studentId),
  });
  const invalidate = () =>
  ...
  const form = useForm<DocValues>({
    ...
    defaultValues: { title: "", type: "BIRTH_CERTIFICATE" },
  });

  const upload = useMutation({
    mutationFn: (values: DocValues) =>
      studentsApi.uploadDocument(studentId, { ...values, file: file! }),
    onSuccess: () => {
  ...
  const remove = useMutation({
    mutationFn: (docId: string) =>
      studentsApi.removeDocument(studentId, docId),
    onSuccess: () => {
  ...
  if (docs.isError)
    return <ErrorState error={docs.error} onRetry={() => void docs.refetch()} />;

  return (
    <div className="space-y-3">
      <Can permission="student.document.manage">
      ...
      {docs.data.length === 0 ? (
        <p className="rounded-lg border p-8 text-center text-sm text-muted-foreground">
          No documents uploaded yet (birth certificate, transfer certificate,
          previous marksheet…).
        </p>
      ) : (
      ...
      >
        <div className="space-y-2">
          <Label htmlFor="sdoc-title">Title</Label>
          <Input id="sdoc-title" {...form.register("title")} />
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(v) =>
              form.setValue("type", v as StudentDocumentType)
            }
          >
