...
import { staffApi, type StaffDocument } from "@/lib/api/staff";
import {
  staffDocumentSchema,
...
export function DocumentsTab({ staffId }: { staffId: string }) {
  ...
  const docs = useQuery({
    ...
    queryFn: () => staffApi.listDocuments(staffId),
  });
  const invalidate = () =>
  ...
  const upload = useMutation({
    mutationFn: (values: StaffDocumentValues) =>
      staffApi.uploadDocument(staffId, { ...values, file: file! }),
    onSuccess: () => {
  ...
  const remove = useMutation({
    mutationFn: (docId: string) => staffApi.removeDocument(staffId, docId),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-3">
      <Can permission="staff.document.manage">
      ...
      <FormDialog
        ...
        description={file ? `File: ${file.name} (${formatBytes(file.size)})` : undefined}
        form={form}
