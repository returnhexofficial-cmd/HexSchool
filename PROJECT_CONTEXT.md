...
| e2e suites run serially (`maxWorkers: 1` in jest-e2e.json) | every suite (14 as of M14) shares ONE dev DB/Redis/Mailpit — parallel workers caused cross-suite flakes. The corollary, learned in M14: a suite must not assert on process-wide state such as heap, which accumulates across the run | M06 |
...
- **M14:** `EXAM_RESULT_GATE` is a no-op — publishing an exam is currently *allowed* and logged rather than gated. **M15 must bind it** or results can be published before they are processed.
- **M14:** `EXAM_DUES_GATE` is a no-op, so `exam.admit_card_block_dues` is inert until M16 binds a ledger.
- **M14:** detaching a class / deleting a paper has no "blocked once marks exist" check — the guard slot awaits the M15 marks table (same shape as the M06 subject-removal slot).
- **M14:** exam-type `weight` is validated 0–100 individually; that a *combined set* sums to 100 is left to M15, the only module that knows which types a report card merges.
...
- **Cross-module (test isolation):** health-check e2e assertions must not depend on the Jest worker's memory. `health.e2e-spec` asserted `memory_heap`/`memory_rss` were `up`; those probes measure the test process, so the assertion went red the moment a 14th suite was added. It now asserts the dependency probes strictly and only checks the memory probes exist.
