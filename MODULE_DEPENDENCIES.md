...
graph TD
    ...
    M27 --> M29
    M28 --> M29
    M29 -. replaces M18's reports catalog .-> M18
...
| 29 Reports & Analytics v2 ✅ | 18, 20, 21 (all Phase 1–2 data) | **The second LEAF AGGREGATOR, and the counterpart to M18's.** PortalModule proved that a module importing many feature modules and imported by none is cycle-free by construction; this is the same argument from the reporting end. It imports **eleven** feature modules — Attendance, Result, Fee, Accounting, Hr, Library, Transport, Inventory, Hostel, Document, Community — and every one of them purely for its **exported report service**, so a spreadsheet prints the module's own numbers rather than a second query that drifts from the screen (the M12 reports/export split, applied across module boundaries). **That is precisely what M24's row predicted** when it exported `InventoryReportsService` although nothing imported InventoryModule at the time; all eleven are consumed now, and the eight analytics notes M22–M28 left are answered. Three services had to be made reachable, all additively: `FeeReportsService` (M16) and `CommunityReportsService` (M28) were **added to their modules' exports**, while `AttendanceSettingsService` was **re-provisioned** instead — it needs `SettingsService` alone, and re-provisioning beat widening M12's surface for one threshold (the M07/M19/M23/M27/M28 convention). `AnalyticsRepository` is the **ninth use** of the narrow-repository pattern (M12 `EmployeeDirectoryRepository` → M17 → M18 → M19 → M22 → M23 → M24 → M28) and the use where it matters most — the alternative is importing twelve modules to pull one number each — and it also carries the three raw materialized-view reads, since Prisma has no model for a view. Also imports School (settings), Rbac (the engine resolves a principal's codes for a run that has no request behind it), Storage (report files) and Communication (`NotificationService.send`, the M17 single-entry-point rule) for the two new codes `REPORT_READY` / `REPORT_SCHEDULE_FAILED`. **Its one BREAKING CHANGE is an edge pointing backwards**: `@Controller('reports')` moved out of PortalModule, whose `report.registry.ts`, `ReportsService` and `ReportsController` are deleted — two controllers on one path would collide and two registries would be two sources of truth. `GET /reports` answers at the same URL with a superset of its old payload, so nothing at the HTTP layer must react. **Leaves no no-op hooks.** For 30: `report_runs` and the MV refresh are the first entries its job log and system console will want, and the module exports `ReportEngineService`, `ReportCatalogService`, `ExecutiveAnalyticsService`, `MaterializedViewService` and `AnalyticsSettingsService` for it on the M21/M24 rule; for 32's AI analytics: the executive panels and the three MVs are the feature source an at-risk model would read. |
