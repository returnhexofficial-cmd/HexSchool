...
> **Last updated:** 2026-07-21 (Module 11 complete — enrollments with partial-unique roll/session invariants, single/bulk enroll + capacity override, section transfer, renumber, promotion wizard build/preview/execute/rollback; EnrollmentModule exports the canonical roster service; StudentModule now exports StudentStatusHistoryRepository + IdCardService; M06 section delete-guard + M09 section batch ID cards closed)
...
- **M09:** `attendance-history`/`performance-history` return an empty self-describing shape until M12/M15.
...
- **M11:** promotion rollback guard is a no-op hook until attendance (M12) / marks (M15) exist — must start blocking then.
- **M11:** `enrollments.enrollment_date` for mid-year `TRANSFERRED_IN` students is stored but not yet consumed (attendance/fees proration lands with M12/M16).
