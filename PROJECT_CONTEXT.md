...
> **Last updated:** 2026-07-22 (Module 15 complete — the four-eyes mark lifecycle, five dependency-free calculation engines, idempotent processing runs with durable progress, versioned publication, report cards/tabulation/transcripts, analytics and combined results; `EXAM_RESULT_GATE` is live, the grade scale now freezes at first PROCESSING rather than at PUBLISH, and the M06/M11/M14 "once marks exist" guards plus M09 `performance-history` are all armed)
...
- **M14:** `EXAM_DUES_GATE` is a no-op, so `exam.admit_card_block_dues` is inert until M16 binds a ledger.
