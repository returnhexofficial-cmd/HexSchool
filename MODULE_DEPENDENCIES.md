...
- After 18: ~~**19 (Website)** ∥ **20 (Accounting)** ∥ **21 (HR & Payroll)**~~ — all done. ~~**22**~~ ~~**23**~~ ~~**25**~~ ~~**24**~~ ~~**26**~~ done; **27–28** are mutually independent and can run in either order (M25, M24 and M26 exercised the band in that order and nothing crossed between any of them); only **29** waits on 21 (and now also on 22 for submission analytics, 23 for reading history, 25 for route-level cost analytics, 24 for consumption analytics and asset depreciation, and 26 for occupancy-over-time and cost-per-bed).
